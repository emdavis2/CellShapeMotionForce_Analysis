...
save_path = './figures/HMM_{}_WT'.format(obs_param)
...
plt.savefig('{}/WT_count_in_transitions.pdf'.format(save_path),bbox_inches='tight',format='pdf')
plt.clf()
...
plt.savefig('{}/WT_direction_of_transition.pdf'.format(save_path),bbox_inches='tight',format='pdf')
plt.clf()
...
# data_bp = pd.concat([wt_state0_df,wt_state1_df]).reset_index()
# save_path_fig = '{}/stripplots'.format(save_path)
# #check to see if the path exists, if not make the directory
# if not os.path.exists(save_path_fig):
#   os.mkdir(save_path_fig)
# for column in numeric_wt_state0_df.columns:
#     print(column)
#     stripplot_hmm(column, data_bp, save_path_fig, '{}_stripplot'.format(column))
#     plot_dist(column, wt_state0_df, wt_state1_df, 'Low Force', 'High Force', save_path_fig, column + '_WT_distribution') 


# data_bp = pd.concat([wt_state0_mean_summary_df,wt_state1_mean_summary_df]).reset_index()
...
#     print(column)
#     stripplot_hmm(column, data_bp, save_path_fig, '{}_stripplot'.format(column)) 
#     plot_dist(column, wt_state0_mean_summary_df, wt_state1_mean_summary_df, 'Low Force', 'High Force', save_path_fig, column + '_WT_mean_distribution') 


# data_bp = pd.concat([wt_state0_median_summary_df,wt_state1_median_summary_df]).reset_index()
...
#     print(column)
#     stripplot_hmm(column, data_bp, save_path_fig, '{}_stripplot'.format(column)) 
#     plot_dist(column, wt_state0_median_summary_df, wt_state1_median_summary_df, 'Low Force', 'High Force', save_path_fig, column + '_WT_median_distribution')
...
####Median Plots###
save_path = './figures/HMM_{}_WT/median_segment_stripplots/pdf_vp'.format(obs_param)
numeric_segment_df_all = segment_df_all.select_dtypes(include=np.number)

if not os.path.exists(save_path):
  os.mkdir(save_path)

colors_for_state_plot = [(11/256,218/256,81/256), (242/256,140/256,40/256)]
...
# ####Corr plots###
# save_path = './figures/HMM_{}_WT/corr_plots'.format(obs_param)

# #check to see if the path exists, if not make the directory
# if not os.path.exists(save_path):
#   os.mkdir(save_path)

# A = numeric_wt_state0_df
# B = numeric_wt_state0_df
# unique_pairs = list({tuple(sorted((a, b))) for a in A for b in B})

# for pair in unique_pairs:
#     column1 = pair[0]
#     column2 = pair[1]
#     plt.scatter(wt_state0_df[column1],wt_state0_df[column2],color='tab:blue',alpha=0.25,label='low force')
#     plt.scatter(wt_state1_df[column1],wt_state1_df[column2],color='tab:red',alpha=0.25,label='high force')
