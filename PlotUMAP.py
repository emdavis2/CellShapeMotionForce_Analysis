...
save_path = './figures/UMAP_shapeonly'
...
pooled_df = pd.concat([pd.concat(arpc2ko_cells, ignore_index=True), pd.concat(wt_cells, ignore_index=True)], ignore_index=True)
...
pooled_arpc2ko_df = pooled_df[pooled_df['type'] == 'ARPC2KO']

plt.scatter(
    embedding[:, 0],
    embedding[:, 1],
    c=[sns.color_palette()[x] for x in pooled_df.type.map({"ARPC2KO":0, "WT":1})])
plt.gca().set_aspect('equal', 'datalim')
plt.title('UMAP projection of the cell dataset - colored by type', fontsize=24);

type_colors = {"ARPC2KO": 0, "WT": 1}
palette = sns.color_palette()
# Create custom legend
legend_elements = [
    Patch(facecolor=palette[type_colors["ARPC2KO"]], label='ARPC2KO'),
    Patch(facecolor=palette[type_colors["WT"]], label='WT')
]
plt.legend(handles=legend_elements, title="Cell Type")

plt.savefig('{}/celltype_umap.png'.format(save_path),bbox_inches='tight')
plt.clf()

x_bins=np.linspace(-8,15,40)
y_bins=np.linspace(-5,15,40)
plt.hist2d(pooled_wt_df['umap_x'],pooled_wt_df['umap_y'],bins=[x_bins,y_bins])
plt.colorbar(label='Counts')
plt.savefig('{}/wt_hist_umap.png'.format(save_path),bbox_inches='tight')
plt.clf()

plt.hist2d(pooled_arpc2ko_df['umap_x'],pooled_arpc2ko_df['umap_y'],bins=[x_bins,y_bins])
plt.colorbar(label='Counts')
plt.savefig('{}/arpc2ko_hist_umap.png'.format(save_path),bbox_inches='tight')
plt.clf()
